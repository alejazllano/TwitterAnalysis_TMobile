...
for (i in c("tidytext", "SnowballC", "dplyr","tidyverse", "hunspell","topicmodels", "textstem", "ggplot2", "ldatuning", "utf8", "wordcloud2", "wordcloud", "RColorBrewer","flexdashboard")){  if (!require(i, character.only=TRUE)) install.packages(i, repos = "http://cran.us.r-project.org")
  require(i, character.only=TRUE)
}
...
load('C:/Users/azambranollano/OneDrive - IESEG/Alejandra/Social Media Analytics/Group Project/Final/tmobile_company.RData')

#Tweets generated from different users  are stored as "tmobile_users.RData"

load("C:/Users/azambranollano/OneDrive - IESEG/Alejandra/Social Media Analytics/Group Project/Final/tmobile_users.RData")
...
tmobile_company2<- tmobile_company2 %>% left_join(sent_tweet, by='status_id') %>%
  arrange(sentiment)
colnames(tmobile_company2)
sum(is.na(tmobile_company2$sentiment))
tmobile_company2 <-tmobile_company2%>%drop_na(sentiment)


save(tmobile_company2, file = "C:/Users/fceli/Desktop/BIG DATA/Semester 2/1. Social media analytics/SMA-master/Group assignment/tmobile_company3.RdDta")
...
tmobile_company3<-tmobile_company2
...
tmobile_company3$wday=sapply(tmobile_company3$created_at, function(x) {p=as.POSIXlt(x);p$wday})
...
save(tmobile_users2, file = "C:/Users/fceli/Desktop/BIG DATA/Semester 2/1. Social media analytics/SMA-master/Group assignment/tmobile_users2.Rdata")
...
disgusthesh <- get_sentiments("nrc") %>%
  filter(sentiment == "disgust")
...
#label a tweet with the hour
tmobile_users2$hour=sapply(tmobile_users2$created_at, function(x) {p=as.POSIXlt(x);p$hour})
#label a tweet with a number corresponding to the day of the week
